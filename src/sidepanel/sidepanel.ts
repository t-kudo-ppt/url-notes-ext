...
const persist = debounce(async () => {
  ...
  await setNote(note);
  const idx = allNotesCache.findIndex((n) => n.scope === currentScope && n.key === currentKey);
...
function renderList(notes: Note[]) {
  if (notes.length === 0) {
    listContainer.innerHTML = '<div class="item" aria-live="polite">メモがありません</div>';
    return;
  }
  const html = notes
    .map((n) => {
      ...
      return `
      <div class=\"item\" data-key=\"${encodeURIComponent(n.key)}\" data-scope=\"${n.scope}\">
        <div class=\"line1\">
...
listContainer.addEventListener('click', async (ev) => {
  ...
  if (target instanceof HTMLButtonElement && target.dataset.del === '1') {
    ...
    if (scope && key && confirm('このメモを削除しますか？')) {
      ...
      allNotesCache = allNotesCache.filter((n) => !(n.scope === scope && n.key === key));
      await refreshList(searchInput.value.trim());
    }
    return;
  }
...
async function handleImport(file: File) {
  ...
  for (const n of bundle.notes) {
    ...
    if (typeof (n as any).key !== 'string' || typeof (n as any).scope !== 'string') continue;
    if (typeof (n as any).content !== 'string' || typeof (n as any).updatedAt !== 'number')
      continue;
...
function selectTab(which: 'edit' | 'list') {
  ...
  (viewEdit as any).dataset.active = String(isEdit);
  (viewList as any).dataset.active = String(!isEdit);
}
